...
0. 베타 배포 게이트(P0 보안·권한·프로비저닝 + P1) — 45차: **P0** ① 프로덕션 Mock 차단(`isProdMisconfigured`=`PROD && !isSupabaseConfigured` → 데모 계정 시드 DEV 전용, LoginPage "서비스 설정 미완료" 차단, mockLogin/Signup 방어) ② Premium Fan 없음 확인(코드 부재) ③ 역할 매핑 `mapDbRole`(superadmin/staff/admin→관리자, club/club_admin→Club Account, 그외→fan; `ADMIN_ROLES=[admin,superadmin,staff]`, `CLUB_ROLES=[club,club_admin]`) ④~⑥ SUPABASE_SETUP.md 프로비저닝(마이그레이션 0001~0027 표·Edge 9종 표·관리자/구단 계정 발급·RLS 검증·health-check) + `0027_roles_and_admin_members.sql`(role enum 확장·is_admin 보강·`admin_list_members`/`admin_set_member_deactivated` RPC). **P1** ⑦ 관리자 회원관리 Supabase 실데이터(`membersRepo` RPC/Mock 폴백) ⑧ 데모 데이터 배지(`DemoBadge` — 리그/뉴스/구단대시보드) ⑨ 본인인증 베타 범위=이메일 인증(실 업체 미설정 시 `requiresIdentityVerification`=false·설정/게이팅/버튼 숨김) ⑩ 의견 사진 업로드 제거 ⑪ 리포트 전달 PDF만 ⑫ 구독/결제=B2B 구단 기준(팬 무료) ⑬ 코드 정리(App.jsx/App.css/log-in page.docx/tmp 삭제·README 교체·favicon 통일) ("Add beta deployment gate: security, roles, provisioning").
0. 설정 페이지 UI 단순화(4카드 통합) — 44차: `SettingsPage.jsx` 10카드 → **4카드**로 통합(기능 불변, UI만). **① 내 프로필**(아바타·닉네임·이메일·응원팀·성별·나이대·가입일 읽기전용 + **단일 [프로필 수정]** 버튼 → 인라인 펼침으로 닉네임·응원팀·성별·나이대·사진 편집; `changeGender` auth 추가, `changeNickname`/`setSelectedTeam`/`changeAgeGroup`/`saveAvatar` 재사용, 닉네임 90일 쿨다운 유지, 팀 변경 시 새 구단 설정으로 이동). **② 앱 설정**(언어·테마 ── 브라우저 알림 ── 알림 설정, `.st-subsec`/`.st-div`). **③ 계정 및 보안**(현재기기·이메일인증·본인인증·인증기관·비밀번호변경·로그아웃 + 회원탈퇴는 하단 `.st-danger` 위험영역). **④ 서비스 정보**(앱버전·소개·개인정보·약관). 이메일 중복·프로필 수정 버튼 2개·응원팀 별도카드 제거, 카드 padding/margin 축소로 스크롤 길이 단축. ProfileEditPage 라우트는 유지(설정 링크만 제거) ("Simplify settings page into four cards").
0. 운영 전 UX 2차(랭킹·닉네임·나이대·다국어) — 43차: (1) **관리자 다국어 마무리** — KPI 카테고리 표시명(`admin.kpiCat.<key>`, 분류 로직은 canonical 유지) + 남은 팀명(ClubExecutive `exec-sub`) 영문화 → 영어 모드 UI 한글 잔존 제거(남은 한글=닉네임·의견/AI 콘텐츠 등 데이터뿐). (2) **닉네임 안내·"확인 중" 가독성** — 원인은 고정 다크 signup 카드에 `--ink` 미정의 → `color-mix(var(--ink)…)` 무효 → 검정. `var(--ink,#FFFFFF)` 폴백으로 밝게(78%) 표시. (3) **랭킹 안내 위치** — "전체 랭킹 TOP 50" 옆으로 이동(`.fr-panel-title` 내 `.fr-update-note`). (4) **랭킹 20/50 더보기** — 기본 1~20위(포디움3+목록17), `expanded` state로 `더보기`→50위→`접기`(점수 로직 불변). (5) **자기순위 활동점수 반영 안내 삭제**(`fr-pending` 문구 제거, activityScore 로직 유지). (6) **설정 프로필 UX** — 별도 나이대 카드 제거, 프로필 정보 카드에 이메일·가입일·성별·나이대 표시 + "프로필 정보 수정" → 모달(별도 수정 화면)에서 나이대 칩 선택·저장·성공/실패 메시지(`changeAgeGroup`, 닉네임 쿨다운 무관). 알림 권한분리·스크롤(42차)은 유지 ("Fix ranking notifications i18n and profile settings UX").
0. 운영 전 UX/알림/다국어/설정 버그 수정 — 42차: (1) **관리자 다국어 보완** — 하드코딩 한글 제거(카운트 단위 `건`→`admin.unit.count(N)`, Club Action KPI 요약, 리포트 placeholder, 뉴스 기본 라벨) + **관리자/구단 화면 팀명 `teamName(team,lang)` 로 영문화**(AdminMembers/Opinions/News/Notices/Dashboard/ClubActions/ActionTracker/Customers/ReportDocs/ClubExecutive). (2) **회원가입 닉네임 안내 가독성** — `.nick-status` 폰트 13.5px·대비↑(`color-mix(--ink 68%)`)·여백↑. (3) **팬 랭킹 안내** — `rank.updateNote`("랭킹은 매일 00시에 업데이트") `.fr-update-note`. (4) **관리자 알림 권한 분리** — 운영 시스템 알림(`pushMockNotification audience:'admin'` — Integration/League/News fetch 실패)은 **관리자에게만**(Mock=`isAdmin()` 필터, Supabase=admins user_id 타깃). fan/club 미노출. (5) **알림 스크롤** — `.fc-bell-list max-height:min(65vh,400px)`(≈5개, 6+ 내부 스크롤). (6) **설정 나이대 수정** — `changeAgeGroup()`(닉네임 쿨다운 무관) + SettingsPage 칩 선택·저장·성공/실패 메시지 ("Fix i18n notifications ranking and age settings").
...
- **회원가입(P0, ✅ 완료 2026-07-22)**: 
  - P0-1 이메일 OTP: `send-email-code` Edge(RESEND) + `EMAIL_FROM=noreply@fancluv.com`(도메인 검증 완료, SPF/DKIM) + 실 브라우저 Gmail 수신 확인.
  - P0-2 닉네임 중복: `nickname_available()` SECURITY DEFINER RPC(0073) + 정규화 UNIQUE 인덱스(0072).
  - P0-3 이메일 중복 + OAuth: `email_available()` RPC(0074) + `complete-signup` Edge(OAuth/기존 계정 하드 차단, 최근 shell만 복구).
  - 프로덕션 배포: 0072/0073/0074 마이그레이션 + send-email-code Edge 재배포 + EMAIL_FROM 설정(2026-07-22 11:36 UTC).
  - OTP 보안: 5회 브루트포스 제한·10분 TTL·해시 저장. 닉네임/이메일 정규화(NFC+trim+lower) UNIQUE. Race ×100 무손실 실증.
- **Release Gate 요약(프로덕션 확정)**: Email Provider ✅ / Edge ✅ / DB ✅ / Migration(0072-0074) ✅ / Env ✅ / Realtime ✅ / **EMAIL_FROM ✅** · **Google OAuth ⛔**(Dashboard 미활성, 운영 정책 결정 대기) · **Storage ⚠**(public·업로드 제한 미설정) · AI ⚠(OPENAI 미설정) · Observability ⚠(Sentry stash 대기). 상세·해결 절차는 `docs/RELEASE_GATE.md` "Phase 20-C 재평가".
- **메뉴 정책(고정)**: 홈 / 설문 / 팬 의견 / 팀 뉴스 / 경기센터 / AI 인사이트 / 팬 랭킹 / 내 활동. Fan Pulse·Quick Poll·AI 작성지원·Premium 미개발/제거됨. 홈 "우리 팀 실시간" 제거됨.
- **미검증(사용자 협업 필요)**: 모바일 6종 실측(375/390/430/768), Google/Kakao/Naver 실 OAuth E2E → `docs/RELEASE_GATE.md`의 체크리스트로 진행.
