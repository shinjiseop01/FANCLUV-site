...
export default function CreateOpinionPage() {
  ...
  // 경기센터 "경기의견 작성하기" 진입(?vs=상대팀): 새 글 작성 시 제목을 경기 기준으로 프리필.
  //   (opinions 스키마 확장 없이 화면 프리필만 — 사용자는 자유롭게 수정 가능.)
  useEffect(() => {
    if (isEdit) return
    const vs = searchParams.get('vs')
    if (vs) setTitle(prev => prev || `${teamName(team, lang)} vs ${vs} 경기 의견`)
  }, [isEdit, searchParams, team, lang])
